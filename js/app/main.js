    ...
    require(["framework7"], function(Framework7) {
        require(["dpanels"], function(dpanels) {
            ...
            theApp.onPageInit('*', function (page) {
                ...
                mainView.router.loadPage({url:'_SHOP/shop.html', ignoreCache:true, reload:true });
            ...
            theApp.onPageInit('shop', function (page) {
                /**---------------------
                 *
                 * Cart Item Quantity
                 *
                 *---------------------**/
                console.log('HI');
                var q = 'http://127.0.0.1/trendy_api/gc_categories/ajax';
                var q = 'http://trendyapi.rickchew.com/gc_categories/ajax';

                setTimeout(function(){
                  $$.getJSON(q, function (json,status,xhr) {
                    //myApp.hideIndicator();
                    ...
                    for (var i = 0; i < json.categories.length; i++) {
                        if(json.categories[i].image){
                            console.log(json.categories[i].image);
                            imgDisplay = 'http://trendy.rickchew.com/uploads/images/medium/'+json.categories[i].image;
                        }else{
                            imgDisplay = 'http://trendy.rickchew.com/uploads/images/full/eb0a918595a36623d386041558775c40.jpg';
                        }
                        
                    htmlDisplay += ''+
                    '<div class="col-50 tablet-25">'+
                        '<a href="_SHOP/products.html?ts='+$.now()+'&id='+json.categories[i].id+'">'+
                            '<img src="'+imgDisplay+'" alt=""/>'+
                            '<div>'+
                                '<h2>'+json.categories[i].name+'</h2>'+
                                '<span>'+json.categories[i].count_cat+' Items</span>'+
                            '</div>'+
                        '</a>'+
                    '</div><!-- .col-50 | ENDS -->';
                    }
                    $$("#categories-list").html(htmlDisplay);
                    myApp.hidePreloader();
                  })
                },200);
            ...
            theApp.onPageInit('products', function (page) {
                /**---------------------
                 *
                 * Cart Item Quantity
                 *
                 *---------------------**/
                //console.log('HI');
                var cat_id = GetURLParameter('id');
                var q = 'http://127.0.0.1/trendy_api/gc_products/ajax/'+cat_id;
                var q = 'http://trendyapi.rickchew.com/gc_products/ajax/'+cat_id;
                //var fileObj;
                //var imgArr;
                setTimeout(function(){
                  $$.getJSON(q, function (json,status,xhr) {
                    ...
                    for (var i = 0; i < json.products.length; i++) {
                        imgDisplay = 'http://trendy.rickchew.com/uploads/images/medium/b4c31db1d59418554bbadb150c894588.png';
                        $.each( jQuery.parseJSON(json.products[i].images), function( key, value ) {
                          //console.log(value);
                          imgDisplay = 'http://trendy.rickchew.com/uploads/images/medium/'+value.filename;
                          //alert( key + ": " + value );
                        });
                    ...
                    '<div class="col-100 tablet-50">'+
                        '<div class="tc-product">'+
                            ...
                            '<div class="tag off">NEW</div>'+
                            '<div class="details">'+
                                '<div class="head">'+
                                    '<a href="_SHOP/product.html?ts='+$.now()+'&id='+json.products[i].id+'" class="title"><h3>'+json.products[i].name+'</h3></a>'+
                                    '<a href="_SHOP/product.html?ts='+$.now()+'&id='+json.products[i].id+'" class="price">RM '+json.products[i].price_1+'</a>'+
                                '</div>'+
                                '<div class="buttons">'+
                                    '<a href="#"><i class="uiicon-shopping238"></i> Add to Cart</a>'+
                                    '<a href="_SHOP/product.html?ts='+$.now()+'&id='+json.products[i].id+'"><i class="uiicon-web38"></i> Details</a>'+
                                '</div>'+
                            '</div>'+
                        '</div>'+
                    '</div><!-- .col-100.tablet-50 | ENDS -->';

                    }
                    $$("#products-list").html(htmlDisplay);
                    myApp.hidePreloader();
                  })
                },200);
            ...
            theApp.onPageInit('product', function (page) {
                /**---------------------
                 *
                 * Cart Item Quantity
                 *
                ...
                setTimeout(function(){
                  $$.getJSON(q, function (json,status,xhr) {
                    //fileObj = json.products[i].images;
                    //console.log(fileObj);
                    //myApp.hideIndicator();
                    ...
                    //console.log(jQuery.parseJSON('{"8435c3e654c911686fba125f5fd0fbf6":{"filename":"8435c3e654c911686fba125f5fd0fbf6.jpg","alt":"","caption":""},"08be693a264cd931921922a7d0f830ba":{"filename":"08be693a264cd931921922a7d0f830ba.jpg","alt":"","caption":""},"8088997beace8a92d0fc54046283f901":{"filename":"8088997beace8a92d0fc54046283f901.png","alt":"","caption":""}}'));
